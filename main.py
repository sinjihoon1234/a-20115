...
# [3. 영화 선택 기능]
# 영화별 최대 누적관객수를 구해 내림차순 정렬한 뒤 전체 영화명 목록을 추출합니다.
sorted_movies = (
...
# 세 번째 탭: Top 5 영화 누적 관객수 비교 (다중 선 그래프)
with tab3:
    st.subheader("🏆 누적관객수 상위 5개 영화 흥행 추이 비교")
    
    # 누적관객수가 가장 높은 상위 5개 영화의 이름만 뽑아냅니다.
    top5_movies = (
        df.groupby('영화명')['누적관객수']
        .max()
        .sort_values(ascending=False)
    ...
    # 전체 데이터에서 상위 5개 영화 데이터만 필터링합니다.
    top5_df = df[df['영화명'].isin(top5_movies)]
    
    # [3번 그래프: 다중 선 그래프 그리기]
    # color='영화명' 옵션을 주면 영화별로 알아서 색상이 구분되고 범례(Legend)가 생성됩니다.
    fig3 = px.line(
        top5_df,
        x='기준일자',
        y='누적관객수',
    ...
    st.info("💡 **이 그래프로 알 수 있는 것:** 최고 흥행작 Top 5 영화들 간의 관객 모객 속도(기울기), 최종 흥행 규모, 상영 기간 차이를 직접 비교 분석할 수 있습니다.")
